...
async function loadHiborRates() {
    try {
        ...
        if (Array.isArray(data) && data.length > 0) {
            data.forEach(item => {
                if (item.term && item.rate) {
                    // Only take the first (latest) rate for each term
                    if (!hiborMap[item.term]) {
        ...
        if (hiborMap['1M']) {
            // FIX: Ensure the rate is displayed as a percentage (rate * 100)
            document.getElementById('hibor-1m').textContent = (parseFloat(hiborMap['1M']) * 100).toFixed(2) + '%';
            document.getElementById('hibor-3m').textContent = (parseFloat(hiborMap['3M']) * 100).toFixed(2) + '%';
            document.getElementById('hibor-6m').textContent = (parseFloat(hiborMap['6M']) * 100).toFixed(2) + '%';
