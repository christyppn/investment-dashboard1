...
async function loadHiborRates() {
    try {
        ...
        if (data && data.date) {
            // Fix: Ensure the percentage is correctly formatted
            document.getElementById('hibor-1m').textContent = (parseFloat(data.ir_1m) * 100).toFixed(2) + '%';
            document.getElementById('hibor-3m').textContent = (parseFloat(data.ir_3m) * 100).toFixed(2) + '%';
            document.getElementById('hibor-6m').textContent = (parseFloat(data.ir_6m) * 100).toFixed(2) + '%';
