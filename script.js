 ...
 * Assumes market_data_history.json is an object: { "VIX": [history], "HSI": [history], ... }
 */
async function loadGlobalMarkets() {
    const marketData = await fetchData("market_data_history.json");
    const container = document.getElementById("global-markets-container"); // Assuming a new container
    const symbols = ["VIX", "HSI", "N225"]; // Use the keys from sync_data.py
...
function initDashboard() {
    ...
    loadFearGreedIndex();
    loadMarketBreadth();
    loadGlobalMarkets(); // New call for Global Markets
}
