...
def generate_dummy_data(fng_value=None, fng_sentiment=None):
    ...
    if fng_value is not None:
        if fng_value >= 75:
            analysis_text = "市場情緒極度貪婪，建議投資者謹慎，考慮分批獲利了結。"
            rating = "Extremely Greedy"
        elif fng_value >= 55:
            analysis_text = "市場情緒貪婪，投資者信心高漲，建議關注強勢板塊。"
            rating = "Greedy"
        elif fng_value >= 45:
            analysis_text = "市場情緒中性，觀望情緒濃厚，建議保持現金比例。"
            rating = "Neutral"
        elif fng_value >= 25:
            analysis_text = "市場情緒恐懼，可能存在超跌機會，建議尋找優質股的佈局機會。"
            rating = "Fearful"
        else:
            analysis_text = "市場情緒極度恐懼，恐慌情緒蔓延，可能是長線佈局的良機。"
...
if __name__ == "__main__":
    ...
    generate_dummy_data(fng_value, fng_sentiment)
    fetch_hkma_hibor()
    fetch_market_data()
