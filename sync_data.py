...
def get_current_13f_quarter():
    """Dynamically determines the current 13F filing quarter and year."""
    now = datetime.now()
    year = now.year
    month = now.month

    # Determine the current quarter based on the current date
    if month >= 1 and month <= 3: # Q1 (Jan-Mar)
        current_quarter = 1
    elif month >= 4 and month <= 6: # Q2 (Apr-Jun)
        current_quarter = 2
    elif month >= 7 and month <= 9: # Q3 (Jul-Sep)
        current_quarter = 3
    else: # Q4 (Oct-Dec)
        current_quarter = 4

    # Determine the latest available filing quarter
    # Filings are due 45 days after quarter end
    if current_quarter == 1: # Q1 ends March 31, due May 15
        if now > datetime(year, 5, 15): # If past May 15, Q1 of current year is available
            return f"Q1 {year}"
        else: # If before May 15, Q4 of previous year is the latest available
            return f"Q4 {year - 1}"
    elif current_quarter == 2: # Q2 ends June 30, due August 15
        if now > datetime(year, 8, 15): # If past August 15, Q2 of current year is available
            return f"Q2 {year}"
        else: # If before August 15, Q1 of current year is the latest available
            return f"Q1 {year}"
    elif current_quarter == 3: # Q3 ends September 30, due November 15
        if now > datetime(year, 11, 15): # If past November 15, Q3 of current year is available
            return f"Q3 {year}"
        else: # If before November 15, Q2 of current year is the latest available
            return f"Q2 {year}"
    else: # Q4 (current_quarter == 4) ends December 31, due February 15 of next year
        if now > datetime(year + 1, 2, 15): # If past Feb 15 of next year, Q4 of current year is available
            return f"Q4 {year}"
        else: # If before Feb 15 of next year, Q3 of current year is the latest available
            return f"Q3 {year}"
...
def fetch_hkma_hibor():
    ...
    try:
        ...
        final_data = {
            "timestamp": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),         "data_date": data_date,
            "rates": [
...
def fetch_financial_ratios():
    """Fetches key financial ratios for selected tickers using yfinance."""
    ...
    for ticker_symbol in tickers_to_fetch:
        try:
            ...
            dividend_yield = info.get("dividendYield") # This is usually annual dividend yield
            # For forward P/E, could use info.get("forwardPE")
            # For P/B, could use info.get("priceToBook")
            ...
            logger.info(f"Successfully fetched financial ratios for {ticker_symbol}.")

        except Exception as e:
            logger.error(f"Failed to fetch financial ratios for {ticker_symbol}: {e}")
            financial_ratios_data.append({
                "symbol": ticker_symbol,
                  "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "pe_ratio": None,
                "dividend_yield": None,
                "source": f"Fetch Failed: {e}"
            })
...
def fetch_financial_ratios():
    ...
    save_json(financial_ratios_data, "financial_ratios.json")
    return True
...
def fetch_market_breadth_alpha_vantage():
    ...
    try:
        ...
        return False
def generate_dummy_data(fng_value=None, fng_sentiment=None, vix_value=None, gspc_volume=None, financial_ratios=None):
    ...
    # Incorporate Financial Ratios for valuation insights
    if financial_ratios:
        for ratio_data in financial_ratios:
            symbol = ratio_data.get("symbol")
            pe_ratio = ratio_data.get("pe_ratio")
            dividend_yield = ratio_data.get("dividend_yield")

            if pe_ratio is not None:
                if pe_ratio > 30: # Arbitrary high P/E threshold
                    analysis_text += f" {symbol}的市盈率 ({pe_ratio:.2f}) 偏高，顯示估值可能過熱。"
                    if fng_value is not None and fng_value > 55: # High P/E + Greed
                        analysis_text += " 結合市場貪婪情緒，需警惕估值泡沫風險。"
                        rating = "Overvalued (Greedy)"
                elif pe_ratio < 15: # Arbitrary low P/E threshold
                    analysis_text += f" {symbol}的市盈率 ({pe_ratio:.2f}) 偏低，可能存在價值低估。"
                    if fng_value is not None and fng_value < 45: # Low P/E + Fear
                        analysis_text += " 結合市場恐懼情緒，可能是價值投資的良好時機。"
                        rating = "Undervalued (Fearful)"

            if dividend_yield is not None and dividend_yield > 0.03: # Arbitrary high dividend yield threshold (3%)
                analysis_text += f" {symbol}的股息收益率 ({dividend_yield:.2%}) 較高，對尋求穩定收益的投資者具有吸引力。" 
    f13_data = {
        "timestamp": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        "fund_name": f"伯克希爾·哈撒韋 (BRK.B) 13F 持倉 ({get_current_13f_quarter()})",
        "total_value": round(random.uniform(380.0, 450.0), 2),
        "cash_ratio": round(random.uniform(150.0, 180.0), 2),
        "top_10_ratio": round(random.uniform(80.0, 90.0), 2),
        "quarterly_change": round(random.uniform(-5.0, 5.0), 2),
        "holdings": [
            {"symbol": "AAPL", "value": round(random.uniform(140.0, 160.0), 2), "change": round(random.uniform(-3.0, 3.0), 2)},
            {"symbol": "BAC", "value": round(random.uniform(50.0, 60.0), 2), "change": round(random.uniform(-7.0, 7.0), 2)},
            {"symbol": "KO", "value": round(random.uniform(25.0, 30.0), 2), "change": round(random.uniform(-2.0, 2.0), 2)},
            {"symbol": "AMZN", "value": round(random.uniform(10.0, 20.0), 2), "change": round(random.uniform(-5.0, 5.0), 2)},
            {"symbol": "AXP", "value": round(random.uniform(8.0, 12.0), 2), "change": round(random.uniform(-4.0, 4.0), 2)},
            {"symbol": "CVX", "value": round(random.uniform(5.0, 10.0), 2), "change": round(random.uniform(-6.0, 6.0), 2)},
            {"symbol": "OXY", "value": round(random.uniform(3.0, 7.0), 2), "change": round(random.uniform(-8.0, 8.0), 2)},
            {"symbol": "MCO", "value": round(random.uniform(2.0, 5.0), 2), "change": round(random.uniform(-3.0, 3.0), 2)},
            {"symbol": "VRSN", "value": round(random.uniform(1.0, 3.0), 2), "change": round(random.uniform(-2.0, 2.0), 2)},
            {"symbol": "HPQ", "value": round(random.uniform(0.5, 2.0), 2), "change": round(random.uniform(-1.0, 1.0), 2)},
        ]
    }
    save_json(f13_data, '13f-data.json')
    
    market_sentiment = {
        "timestamp": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),     "consensus": {
...
if __name__ == "__main__":
    ...
    try:
        with open(os.path.join(DATA_DIR, 'financial_ratios.json'), 'r', encoding='utf-8') as f:
            financial_ratios_data = json.load(f)
    except Exception as e:
        logger.error(f"Failed to read financial_ratios.json for AI analysis: {e}")

    fetch_market_breadth_alpha_vantage()
    generate_dummy_data(fng_value, fng_sentiment, latest_vix_close, latest_gspc_volume, financial_ratios_data)
