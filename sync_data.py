...
def get_current_13f_quarter():
    ...
    # Determine the latest available filing quarter based on current date
    # Filing deadlines are approximately Feb 15 (for Q4 previous year), May 15 (for Q1), Aug 15 (for Q2), Nov 15 (for Q3).

    # If current date is before Feb 15, the latest available is Q3 of the previous year
    if now < datetime(year, 2, 15):
        return f"Q3 {year - 1}"
    # If current date is after Feb 15 but before May 15, the latest available is Q4 of the previous year
    elif now < datetime(year, 5, 15):
        return f"Q4 {year - 1}"
    # If current date is after May 15 but before Aug 15, the latest available is Q1 of the current year
    elif now < datetime(year, 8, 15):
        return f"Q1 {year}"
    # If current date is after Aug 15 but before Nov 15, the latest available is Q2 of the current year
    elif now < datetime(year, 11, 15):
        return f"Q2 {year}"
    # If current date is after Nov 15, the latest available is Q3 of the current year
    else:
        return f"Q3 {year}"
