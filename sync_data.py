...
def fetch_hibor_rates():
    ...
    try:
        ...
        # The data is nested, we need to extract the 'record' array
        records = data.get('result', {}).get('records', [])
        
        # Filter for 1M, 3M, 6M terms and save the latest record for each
        hibor_data = []
        target_terms = ["1M", "3M", "6M"]
        
        # We will simplify the output to match the structure the JS expects (an array of objects)
        
        # Create a map to store the latest rate for each term
        latest_rates = {}
        ...
        for record in records:
            ...
            if term in target_terms and term not in latest_rates:
                latest_rates[term] = {
                    "id": record.get('end_of_day_dt'), # Use date as ID
                    "term": term,
                    "rate": record.get('rate'),
                    "timestamp": record.get('end_of_day_dt') # Use date as timestamp
                }
        ...
        if hibor_data:
            ...
            print(f"Successfully fetched and saved {len(hibor_data)} HIBOR rates.")
        else:
            save_json([], "hibor_rates.json")
            print("Warning: HIBOR API returned no valid records.")
